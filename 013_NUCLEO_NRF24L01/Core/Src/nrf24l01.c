...
void CE_Disable(void) //Disable NRF | deActivates RX or TX mode
{
...
CE_Disable();
	...
	nrf24_WriteReg(EN_RXADDR,0); // Not enabling any data pipe 
...
void nrf24_TxMode(uint8_t *Address, uint8_t channel)
{
	...
	nrf24_WriteRegMulti(TX_ADDR,Address,5); //write the TX address. We decided in nrf24_init that address is 5 bytes.
...
uint8_t nrf24_Transmit (uint8_t *data)
{
	...
	if((fifostatus&(1<<4)) && (!(fifostatus&(1<<3))))
	{
		...
		//send command to flush the fifo
